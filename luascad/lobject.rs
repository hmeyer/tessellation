...
impl LObject {
        ...
        where L: hlua::AsMutLua<'a>
    {
        env.set("Box",
            ...
            LObject {
                o: Some(Intersection::from_vec(vec![::truescad_primitive::SlabX::new(x),
        ...
        env.set("Sphere",
                hlua::function1(|radius: Float| {
                                    LObject { o: Some(Sphere::new(radius) as Box<Object>) }
                                }));
        env.set("iCylinder",
                hlua::function1(|radius: Float| {
                                    LObject { o: Some(Cylinder::new(radius) as Box<Object>) }
                                }));
        env.set("iCone",
                hlua::function1(|slope: Float| {
                                    LObject { o: Some(Cone::new(slope, 0.) as Box<Object>) }
                                }));
        env.set("Cylinder",
            ...
            LObject {
                o: Some(Intersection::from_vec(vec![conie, SlabZ::new(length)], smooth)
                            .unwrap() as Box<Object>),
            }
        }));
        env.set("Bend",
